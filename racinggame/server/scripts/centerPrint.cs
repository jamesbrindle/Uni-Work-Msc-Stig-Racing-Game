...
function headsUpTextPrint( %client, %message, %time, %lines )
{
   if( %lines $= "" || ((%lines > 3) || (%lines < 1)) )
      %lines = 1;


   commandToClient( %client, 'HeadsUpTextPrint', %message, %time, %lines );
}
...
function clearHeadsUpTextPrint( %client )
{
   commandToClient( %client, 'ClearHeadsUpPrint');
}
...
function clearHeadsUpTextPrinttAll()
{
	%count = ClientGroup.getCount();
	for (%i = 0; %i < %count; %i++)
	{
		%cl = ClientGroup.getObject(%i);
      if( !%cl.isAIControlled() )
         commandToClient( %cl, 'ClearHeadsUpTextPrint');
   }
}
